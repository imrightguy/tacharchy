# Directs user to Tacharchy GitHub issues
QR_CODE='
█▀▀▀▀▀█ ▄ ▄ ▀▄▄▄█ █▀▀▀▀▀█
█ ███ █ ▄▄▄▄▀▄▀▄▀ █ ███ █
█ ▀▀▀ █ ▄█  ▄█▄▄▀ █ ▀▀▀ █
...
show_log_tail() {
  if [[ -f $TACHARCHY_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$TACHARCHY_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Tacharchy installation stopped!"
  show_log_tail
  ...
  gum style "$QR_CODE"
  echo
  gum style "Get help from the community via GitHub issues: https://github.com/rightguy/tacharchy/issues"
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/tacharchy/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$TACHARCHY_INSTALL_LOG_FILE"
      else
        tail "$TACHARCHY_INSTALL_LOG_FILE"
      fi
      ;;
    "Upload log for support")
      tacharchy-upload-log
      ;;
    "Exit" | "")
      exit 1
      ;;

...
Input (wallpaper or seed color)
  ...
  → palette applied to TMS shell via IPC
...
```bash
tacharya theme set /path/to/wallpaper.jpg    # Dynamic: palette from wallpaper
tacharya theme set tokyo-night                # Named theme (uses seed color)
tacharya theme list                           # Show available themes
tacharya theme set --seed #FF6B00             # Direct seed color
tacharya theme set --light                    # Toggle light mode
tacharya theme set --dark                     # Toggle dark mode
```
...
| App | Config | Format |
|---|---|---|
| TMS shell | QML/JS (StockThemes) | Material You roles |
...
```bash
tacharya refresh ghostty     # Reload one app
...
2. Register it: `tacharya theme add my-theme --seed #FF6B00`
3. Optionally set light/dark mode defaults
4. Apply: `tacharya theme set my-theme`
...
- [DankMaterialShell](https://github.com/AvengeMedia/DankMaterialShell) — TMS theme application engine
